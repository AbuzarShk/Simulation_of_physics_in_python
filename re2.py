...
red_light = sphere(pos=vector(-1.9,2,0),radius=0.1,
                   color=color.red,vel=vector(1,-2,0),
                   make_trail=True,trail_type="points")

green_light = sphere(pos=vector(1.9,2,0),radius=0.1,
                  color=color.green,vel=vector(-1,-2,0),
                  make_trail=True,trail_type="points")
...
while(time<4):
  ...
  if (red_light.pos.y<mirror.top):
    red_light.vel.y *= -1
  if (green_light.pos.y<mirror.top):
    green_light.vel.y *= -1
  time += dt
